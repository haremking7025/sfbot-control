...
### ด่านที่เพิ่มรอบ 22 ก.ย. 2569 (หน้าตาที่ผู้ใช้เห็น)
ชุด `run_harness_suite.py` ครอบเพิ่มแล้ว — ถ้าข้อใดแดง ห้ามปล่อย:
- `_logo_verify` G8: ทุกจุดที่ตั้งไอคอนหน้าต่าง/ทาสก์บาร์ต้องใช้ `iconphoto(True, …)`
  (ไอคอนคลาส = ตัวที่ทาสก์บาร์วาด) — "มีตัวแปรแต่ไอคอนไม่ขึ้น" เคยผ่านมาแล้ว
- `_scrollbar_theme_verify` S7: "กรอบใน" ของ `ScrolledText` ต้องถูกทาเป็นสีธีม (เดิมเป็นสีระบบ = เทา)
- `_boot_smoke`: สแกนสีพื้นทั้งโปรแกรมทุกแท็บ — ห้ามมีวิดเจ็ตนอกจานสีของธีม (พื้นหลังเพี้ยน)
  พร้อมตัวควบคุมเชิงลบที่ยัดสีหลุดลงจอจริง
...
- [ ] **โลโก้ใน EXE ต้องผ่านทุกครั้ง (22 ก.ย. 2569)** — `build_client.bat` หยุดเองถ้าไม่พบ
      `assets/sf_logo.ico`/`sf_logo.jpg` และหลังบิ๊วด์จะรัน `tools/check_exe_icon.py "<exe>"`
      ดึงไอคอนจริงออกมาเทียบพิกเซลกับโลโก้ (ไอคอนเริ่มต้นของ Tk/PyInstaller = ไม่ผ่าน)

  ```bash
  venv/Scripts/python.exe -X utf8 tools/check_exe_icon.py "dist/SFKeyword/SFKeyword_v<ver>.exe"
  venv/Scripts/python.exe -X utf8 tools/_logo_verify.py      # โลโก้ทุก popup/หน้าต่างย่อย + เหตุของไอคอน
  ```

      > กติกา: ทุก popup/หน้าต่างย่อยติดโลโก้ · หัวแก้บไม่ติดโลโก้ (อยู่ในแถบข้างแทน) · ไฟล์โลโก้
      > ต้องหาเจอทั้งใน EXE และตอนรันจากซอร์ส (`_logo_source_path`) · ห้ามแคชค่า None ของรูป
      > (เคยทำโลโก้หายทั้งโปรเซส) · ไอคอนจาก Win32 เป็น BGRA แต่ PIL เป็น RGBA — ต้องสลับช่องก่อนเทียบ
