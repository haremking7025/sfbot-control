...
- [ ] **ด่านเปิด EXE จริง** — ยกขึ้นเป็นด่านอัตโนมัติแล้ว (`tools/_packaged_exe_verify.py`):
      ...
      > ⚠️ **ถ้าเปิดโปรแกรมค้างอยู่ ตอนรันด่านนี้** — แอปใช้ single-instance mutex ชื่อเดียวกัน
      > ทุกเวอร์ชัน ⇒ ตัวที่เทสต์สั่งเปิดจะ *ออกทันที* แล้วเทสต์จะไปวัดหน้าต่างของ *ตัวเก่าที่คุณเปิด*
      > (หรือแย่กว่านั้น: สั่งซ่อนหน้าต่างของคุณ) ⇒ ผลเขียว/แดงไม่เกี่ยวกับ build ใหม่เลย
      > ตัวรันจึง **ปิดตัวที่ค้างก่อน** แล้วรายงานว่าปิดใครไป (PID) — ตรรกะอยู่ที่ `tools/e2e_guard.py`
      > ตรวจได้โดยไม่ต้องเปิดโปรแกรมเลยด้วย `tools/_e2eguard_verify.py`
      > ถ้าไม่อยากให้ปิด ให้ตั้งสวิตช์นี้ ⇒ ตัวรันจะ **ปฏิเสธจะรัน** แทนที่จะไปแตะหน้าต่างของคุณ:

      ```bat
      set SFKEYWORD_E2E_KEEP_OPEN=1
      ```
...
- [ ] **ปุ่ม "ย้อนกลับเวอร์ชัน" ในแอปยังใช้ได้กับรุ่นที่เพิ่งปล่อย** — ผู้ใช้กดเองได้ที่
      แท็บ ⚙ ตั้งค่า (ไม่ต้องรอผู้ดูแล) ⇒ ต้องมี **แถวใน `docs/release_history.csv` ที่มี sha256**
      ของทุกรุ่นที่อยากให้ย้อนได้ (รุ่นที่ไม่มี sha = กดติดตั้งไม่ได้ พร้อมบอกเหตุผลบนจอ)
      → ตรวจเร็ว: `tools/_release_consistency_verify.py` (ด่านบังคับอยู่แล้ว) และ
      `tools/_approllback_verify.py` (ตรรกะคัดกรองรุ่น + ลำดับตรวจก่อนติดตั้ง)

- [ ] **ย้อนไปรุ่นก่อน 1.3.6 (ยุค SFBOT) ต้องไม่มีสะดุด** — รุ่นเหล่านั้นใช้โฟลเดอร์ข้อมูล
      `%LOCALAPPDATA%\SFBOT` คนละที่ ⇒ หน้าต่างย้อนเวอร์ชันจะโชว์ช่องติ๊ก
      "คัดลอกบัญชี/Cookie/ค่าตั้ง" (เปิดไว้ให้) · ก่อนเขียนทับจะ **สำรองโฟลเดอร์เดิมทั้งก้อน**
      เสมอ · สำรองไม่ผ่าน = ไม่คัดลอกสักไฟล์ (รายงานให้ผู้ใช้เห็น ไม่เงียบ)
      → ตรวจเร็ว: `tools/_legacyfolder_verify.py` (แผนคัดลอก L1–L3 · เส้นทางสำเร็จ/ล้มเหลว
      V1–V3/F1–F2 · ตัวควบคุมเชิงลบ N1–N2 · การต่อสายเข้าหน้าจอ W1–W2)
