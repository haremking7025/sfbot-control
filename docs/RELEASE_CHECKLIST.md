...
- [ ] **ทบทวนคำที่โปรแกรม "อ่านผลไม่ได้"** — ทุกครั้งที่อ่านคำตอบของเว็บไม่ได้ (คำไม่เข้าตาราง
      ...
      > เติมคำแล้ว **ต้อง** รันชุดด่านต่อ: `SESSION_LOST_PATTERNS` (`core/web_signals.py`) ·
      > `WEB_FAIL_REASONS` (`core/inventory_flow.py`) · `KEYWORD_ERROR_PATTERNS` (`core/http_flow.py`)
      > · ด่านที่ผูกกับสามจุดนี้คือ `tools/_unknownmsg_verify.py` (อยู่ในชุด harness — build รันเอง)
