...
| ⚙ ตั้งค่า | หน่วงระหว่างล็อกอิน (เว้นจังหวะล็อกอินไอดีเดียวกัน), เปิดพร้อมวินโดวส์, 🔒 รหัสล็อก (ล็อกส่วนไหนก็ได้ · เพดานไอดี · เปลี่ยน/ลบรหัส) |
...
0. `tools/release_publish.py --apply` **เริ่มด้วยด่านกันพลาด 4 ข้อ** — รุ่นใหม่กว่าเดิม · ปิด
   ปรับปรุงอยู่หรือไม่ · ชุดด่านเขียว · EXE เปิดขึ้นจริง (ถอยรุ่นไม่ได้แล้ว ⇒ ไม่มีธงข้ามด่าน ①②)
...
5. สำรองรุ่นที่ยังอยู่บน GitHub ลงเครื่อง (`tools/prune_releases.py --backup-only --apply` — ไม่ลบอะไร)
6. ล้างของ (wrapper/script/log/zip/dist/build) + prune release/tag เก่าบน GitHub **เหลือ 2 ตัวล่าสุด**
   (`tools/prune_releases.py --keep-last 2` — กันไว้เผื่อต้องดาวน์โหลด ZIP เดิมมาตรวจซ้ำ)

### ล็อกเครื่องลูกค้าจากศูนย์กลาง (`lock_policy.json`)

เครื่องลูกค้าดึง `lock_policy.json` จาก repo ทุกครั้งที่เปิดโปรแกรม (ก่อนหน้าต่างหลักขึ้น)
แล้ว **ทับค่าล็อกของเครื่องนั้น** ⇒ ล็อก/ถอน/เปลี่ยนรหัส ทำจาก repo ทีเดียวถึงทุกเครื่อง

```jsonc
{
  "revision": 1,                 // เลขรอบไว้ให้ตามงาน (โชว์บนการ์ด)
  "active": true,                // false = ไฟล์นี้ไม่มีผลกับเครื่องเลย
  "pages": ["accounts", "settings", "startup"],   // ส่วนที่จะล็อก (startup = ถามตั้งแต่เปิดโปรแกรม)
  "cap": 0,                      // เพดานไอดี: 0 = ทุกแถว · 5 = ค่าเริ่มต้น · null = ไม่จำกัด · ไม่ใส่คีย์ = ไม่ทับ
  "code": "sha256$…",             // รหัสกลาง (ต้องเป็นค่าแปลงกลับไม่ได้)
  "devices": {                   // รหัสแยกรายเครื่อง (ชนะค่ารวม · ที่เหลือสืบทอดค่ากลาง)
    "37F0-B993-8EC6": { "code": "sha256$…", "cap": 3, "note": "ลูกค้า A" }
  }
}
```

- **รหัสเครื่อง** ดูได้ที่หน้าตั้งค่า → การ์ด 🔒 บรรทัด `🔐 ล็อกจากศูนย์กลาง … รหัสเครื่อง XXXX-XXXX-XXXX`
  (มาจาก MachineGuid ของวินโดวส์ · เสถียรข้ามการเปิดโปรแกรม)
- **สร้างแฮชรหัส**: ตั้งรหัสในเครื่องทดสอบ 1 ครั้ง แล้วอ่านค่า `tools_lock_code` จาก
  `%LOCALAPPDATA%\SFKeyword\config\system.json` — หรือใช้ `core/tool_lock.hash_code("รหัส")`
- **เมื่อนโยบาย active เครื่องลูกค้าแก้เองไม่ได้** (ปิดสวิตช์/ตั้งรหัส/ลบรหัส/เปลี่ยนส่วน/เพดาน
  ถูกปฏิเสธพร้อมบอกเหตุ) · การแก้จะไม่ถูกใช้
- **ดึงไฟล์ไม่ได้ = คงนโยบายเดิม** (ไม่ปลดล็อกให้เอง) · **กู้เมื่อล็อกผิด**: แก้ `active: false` แล้ว push
- ต้องปล่อย build ที่มีกลไกนี้ก่อน (เครื่องที่ใช้รุ่นเก่าจะยังดึงนโยบายไปใช้ไม่ได้)

### ทิศทางการอัปเดต

**ขึ้นรุ่นใหม่เท่านั้น** — ตัวอัปเดตเสนอเฉพาะรุ่นที่ใหม่กว่า และ `update.json` ที่ตั้ง
`force_update: true` จะบังคับให้ทุกเครื่องอัปเดตทันที (ตรวจครั้งเดียวตอนเปิดโปรแกรม)

- **ไม่มีปุ่มย้อนเวอร์ชันในโปรแกรมอีกแล้ว** (ถอดออก 21 ก.ย. 2569 — ดู `docs/WORDING.md`)
  พร้อมกับสคริปต์ฝั่งผู้ดูแลและตัวกันอัปเดตของรุ่นที่ถอยหนี ⇒ สิ่งที่ลูกค้าทำได้คืออัปเดตขึ้นรุ่นใหม่
- ลูกค้าที่ติดตั้ง EXE รุ่นเก่าทับเองยังทำได้ (อยู่นอกโปรแกรม) — ตัวอัปเดตจะเสนอรุ่นใหม่ให้ตามปกติ
- ไฟล์ `.exe.old` (ไฟล์เก่าที่ซ่อนไว้ตอนติดตั้ง) ยังถูกเก็บไว้เป็น **ตัวกันอัปเดตพังเท่านั้น**
  ⇒ คืนไฟล์เดิมได้เมื่อคัดลอกรุ่นใหม่ไม่สำเร็จ ไม่ใช่ทางย้อนเวอร์ชันให้ผู้ใช้
- **ห้ามปล่อยขณะสวิตช์ปิดปรับปรุงเปิดอยู่** — เครื่องลูกค้าปิดตัวเองตอนเปิดโปรแกรม
  ก่อนถึงขั้นตรวจอัปเดต ⇒ ปล่อยไปก็ไม่มีใครได้ · สคริปต์ปล่อยหยุดให้เองถ้าเห็น `active: true`
  (เว้นแต่สั่ง `--allow-maintenance` เอง)
...
| `tool_lock` | กติกาของรหัสล็อก: ลิสต์ส่วนที่ล็อกได้ (ลิสต์เดียวกับเมนู) · แฮชรหัส (`sha256$เกลือ$แฮช`) · เพดานไอดี · การตัดสิน/ข้อความ |
| `lock_policy` | **นโยบายล็อกจากศูนย์กลาง**: ดึง `lock_policy.json` จาก GitHub ตอนเปิดโปรแกรม → เปิด/ปิดล็อก · ส่วนที่จะล็อก · เพดาน · รหัส (แยกรายเครื่องได้) · รหัสเครื่อง (`MachineGuid` แบบสั้น) · fail closed เมื่อดึงไม่ได้ |
...
| `updater` | อัปเดต + kill-switch ผ่าน GitHub (`update.json` / `kill_switch.json`) — ตรวจ **ครั้งเดียวตอนเปิดโปรแกรม** ก่อนหน้าต่างหลักขึ้น ถ้ามีรุ่นใหม่จะเด้งหน้าต่างให้กดอัปเดตเอง (`force_update: true` = บังคับ) · ตรวจขนาด/SHA-256/ชนิดไฟล์ทุกครั้งก่อนติดตั้ง (`verify_update_file` · `download_update` · `apply_update`) · เร่งความเร็ว: keep-alive session ต่อเธรด · ไฟล์ ≥ 8 MB โหลดหลายช่วงพร้อมกัน 4 เส้น (ถอยไปเส้นเดียวเองถ้าเซิร์ฟเวอร์ไม่รองรับ Range) |
...
- **ค่าที่ตั้งในแต่ละแท็บถูกจำข้ามการเปิดโปรแกรม**: หน่วงล็อกอิน · พร้อมกัน (แดชบอร์ด /
  รับไอเทมฟรี / ฝาก-เบิก) · **ต่อบัญชี (ฝาก-เบิก)** · สวิตช์แจ้งเตือน / ทำงานต่อ /
  เปิดพร้อมวินโดวส์ · ตัวกรอง + คำค้นของ log — เปิดโปรแกรมใหม่ได้ค่าเดิมกลับมาครบ
  (ยกเว้นตัวกรองหมวดของหน้าฝาก/เบิก ที่ตั้งใจไม่จำ เพราะเปลี่ยนว่าอะไรจะถูกฝาก/เบิก)
- **หน่วงระหว่างล็อกอิน**: ล็อกอิน HTTP เว้นจังหวะ 0.1 วิ/ไอดี (ค่าเริ่มต้น 0.1 —
  ปรับได้ที่แท็บตั้งค่า และ**ค่าที่เลือกถูกจำไว้ข้ามการเปิดโปรแกรม**) · หน่วงเฉพาะ
...
| อาการ | วิธีแก้ |
|---|---|
| ล็อกอินไม่สำเร็จซ้ำๆ ทั้งที่รหัสถูก | แดชบอร์ด → **🍪 ล้าง Cookie ทั้งหมด** แล้วล็อกอินใหม่ |
| กรอกคีย์เวิร์ดแล้วไม่มีอะไรเกิดขึ้น | ต้องล็อกอินผ่านก่อนเสมอ — ดูการ์ดสรุปผลในแดชบอร์ด |
| บัญชีถูกล็อก 15 นาที (ข้อความ "suspicious activity") | รอ 15 นาที หรือเพิ่ม "หน่วงระหว่างล็อกอิน" ในแท็บตั้งค่า (ค่าที่เลือกถูกจำไว้ใช้ครั้งต่อไป) — อย่ายิงล็อกอินซ้ำเร็วๆ |
| อยากปิดเซสชันที่ค้าง | แดชบอร์ด → **ปิดเซสชันทั้งหมด** แล้วล็อกอินใหม่ |
