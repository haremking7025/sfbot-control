...
1. อ่านเวอร์ชันจาก `sfkeyword_lib/core/constants.py` (`VERSION = "1.3.57"`) ผ่าน `tools/get_version.py`
   → ตั้งชื่อไฟล์เป็น `SFKeyword_v1.3.57.exe` อัตโนมัติ
...
5. `PyInstaller --onefile --noconsole` → ไฟล์ออกที่ **`dist\SFKeyword\SFKeyword_v1.3.57.exe`**
   + สร้าง `SFKeyword_v1.3.57.zip` + คำนวณ SHA-256
