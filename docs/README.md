...
1. อ่านเวอร์ชันจาก `sfkeyword_lib/core/constants.py` (`VERSION = "1.3.61"`) ผ่าน `tools/get_version.py`
   → ตั้งชื่อไฟล์เป็น `SFKeyword_v1.3.61.exe` อัตโนมัติ
...
5. `PyInstaller --onefile --noconsole` → ไฟล์ออกที่ **`dist\SFKeyword\SFKeyword_v1.3.61.exe`**
   + สร้าง `SFKeyword_v1.3.61.zip` + คำนวณ SHA-256
...
- **หน่วงระหว่างล็อกอิน**: ล็อกอิน HTTP เว้นจังหวะ 0.1 วิ/ไอดี (ปรับได้ที่แท็บตั้งค่า —
  ล็อก 15 นาทีเกิดจากใส่รหัสผิดซ้ำในบัญชีเดียวกัน ไม่เกี่ยวกับตัวหน่วงนี้)
...
| บัญชีถูกล็อก 15 นาที (ข้อความ "suspicious activity") | รอ 15 นาที หรือเพิ่ม "หน่วงระหว่างล็อกอิน" ในแท็บตั้งค่า — อย่ายิงล็อกอินซ้ำเร็วๆ |
